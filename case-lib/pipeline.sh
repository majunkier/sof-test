...
# Extract PCM device and optionally channel count from topology if not specified
# Args: $1: topology path
#       $2: pipeline type (playback/capture)
#       $3: variable name to store PCM device (will be set as global)
#       $4: variable name to store channel count (will be set as global, optional)
#       $5: default channel value to check against (optional, if current equals default, override from topology)
# Returns: 0 on success, exits with code 2 if no pipeline found
# Example: func_extract_pcm_from_topology "$TPLG" "playback" "pcm_p" "channel_p" "2"
func_extract_pcm_from_topology()
{
    local tplg="$1"
    local type="$2"
    local pcm_var="$3"
    ...
    [ -z "$tplg" ] && die "No topology specified"

    func_pipeline_export "$tplg" "type:$type"
    if [ "$PIPELINE_COUNT" -gt 0 ]; then
        # Extract PCM device
        local dev=$(func_pipeline_parse_value 0 dev)
        eval "$pcm_var='$dev'"
        dlogi "Extracted $type PCM: $dev"

        # Extract channel count if variable name provided
        if [ -n "$channel_var" ]; then
            local current_channel
            eval "current_channel=\$$channel_var"
            
            # Override channel if it matches default or is empty
            if [ -z "$current_channel" ] || [ "$current_channel" = "$default_channel" ]; then
                local topo_channel=$(func_pipeline_parse_value 0 channel)
                if [ -n "$topo_channel" ]; then
                    eval "$channel_var='$topo_channel'"
                    dlogi "Using $type channel count from topology: $topo_channel"
                fi
            fi
        fi
    else
        dloge "No $type pipeline found in topology"
        exit 2
    fi
}
